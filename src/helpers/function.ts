...
import { UserWallModel, AdminModel } from '../models';
		...
		Token.ensure(token)
			.then(payload => {
				...
				switch ((payload as any).role) {
					case UserRole.usersWall:
						UserWallModel.findOne({ profile: (payload as any).id }).populate('profile').then(client => resolve(client));
						break;
